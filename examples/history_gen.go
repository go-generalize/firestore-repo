...
//go:generate mockgen -source history_gen.go -destination mock/mock_history_gen/mock_history_gen.go
...
// Get 処理中の History の取得処理一切の責任を持ち、これを行う
...
// Update 処理中の History の更新処理一切の責任を持ち、これを行う
...
// StrictUpdate 処理中の History の厳格な更新処理一切の責任を持ち、これを行う
...
// Delete 処理中の History の削除処理一切の責任を持ち、これを行う
...
// DeleteByID 処理中の History の IDから削除処理一切の責任を持ち、これを行う
...
// GetMulti 処理中の History の一括取得処理一切の責任を持ち、これを行う
...
// InsertMulti 処理中の History の一括挿入処理一切の責任を持ち、これを行う
...
// UpdateMulti 処理中の History の一括更新処理一切の責任を持ち、これを行う
...
// DeleteMulti 処理中の History の一括削除処理一切の責任を持ち、これを行う
...
// DeleteMultiByIDs 処理中の History の ID群を元に一括削除処理一切の責任を持ち、これを行う
...
// GetWithTx トランザクション中の History の取得処理一切の責任を持ち、これを行う
...
// GetWithDocWithTx トランザクション中の History の取得処理一切の責任を持ち、これを行う
...
// InsertWithTx トランザクション中の History の登録処理一切の責任を持ち、これを行う
...
// UpdateWithTx トランザクション中の History の更新処理一切の責任を持ち、これを行う
...
// StrictUpdateWithTx トランザクション中の History の厳格な更新処理一切の責任を持ち、これを行う
...
// DeleteWithTx トランザクション中の History の削除処理一切の責任を持ち、これを行う
...
// DeleteByIDWithTx トランザクション中の History の削除処理一切の責任を持ち、これを行う
...
// GetMultiWithTx トランザクション中の History の一括取得処理一切の責任を持ち、これを行う
...
// InsertMultiWithTx トランザクション中の History の一括挿入処理一切の責任を持ち、これを行う
...
// UpdateMultiWithTx トランザクション中の History の一括挿入処理一切の責任を持ち、これを行う
...
// DeleteMultiWithTx トランザクション中の History の一括削除処理一切の責任を持ち、これを行う
...
// DeleteMultiByIDWithTx トランザクション中の History の一括削除処理一切の責任を持ち、これを行う
	...
	if q == nil {
		...
		if req.IsSubCollection != nil {
			for _, chain := range req.IsSubCollection.QueryGroup {
				ref.Query = ref.Query.Where("IsSubCollection", chain.Operator, chain.Value)
			}
		}
